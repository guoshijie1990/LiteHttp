...
public abstract class HttpListener<Data> {
    ...
    private static final int M_SUCCESS_CACHE = 10;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case M_SUCCESS:
                    ...
                    break;
                case M_SUCCESS_CACHE:
                    data = (Object[]) msg.obj;
                    onCacheSuccess((Data) data[0], (Response<Data>) data[1]);
                    break;
                case M_FAILURE:
                    data = (Object[]) msg.obj;
    ...
    public final void notifyCallCacheSuccess(Data data, Response<Data> response) {
        delayOrNot();
        if (disableListener()) {
            return;
        }
        if (runOnUiThread) {
            Message msg = handler.obtainMessage(M_SUCCESS_CACHE);
            msg.obj = new Object[]{data, response};
            handler.sendMessage(msg);
        } else {
            onCacheSuccess(data, response);
        }
        if (linkedListener != null) {
            linkedListener.notifyCallCacheSuccess(data, response);
        }
    }
    ...
    public void onCacheSuccess(Data data, Response<Data> response) {}
