...
public class LiteHttp {
    ...
    private final Set<AbstractRequest<?>> mCurrentRequests = new HashSet<AbstractRequest<?>>();
    ...
    public <T> JsonRequest<T> executeAsync(HttpRichParamModel<T> model, Object tag) {
        JsonRequest<T> request = model.buildRequest();
        request.setTag(tag);
        executeAsync(request);
        return request;
    ...
    public <T> Response<T> execute(AbstractRequest<T> request) {
        ...
        try {
            ...
            if (request.getCacheMode() == CacheMode.CacheOnly) {
                ...
                if (listener != null) {
                    listener.notifyCallCacheSuccess(response.getResult(), response);
                }
            }
     ...
     * {@link AbstractRequest#(RequestFilter)}.
     */
    public interface RequestFilter {
        public boolean apply(AbstractRequest<?> request);
    }
    ...
    public void cancelAll(RequestFilter filter) {
        synchronized (mCurrentRequests) {
            for (AbstractRequest<?> request : mCurrentRequests) {
                if (filter.apply(request)) {
                    request.cancel();
                    HttpLog.v(TAG, request.getTag() + ",url=" + request.getUri() + "取消请求");
                }
            }
        }
    }
